...
use Ziming\LaravelMyinfoSg\Exceptions\MyinfoV5\AuthorizationResponseException;
...
use Ziming\LaravelMyinfoSg\Services\MyinfoV5\AuthorizationCallbackValidator;
...
use Ziming\LaravelMyinfoSg\Services\MyinfoV5\IdTokenProcessor;
...
class MyinfoConnector extends Connector
{
    ...
    public function generateAuthorizationUrl(?string $redirectUri = null): string
    {
        ...
        if (! is_string($effectiveRedirectUri) || $effectiveRedirectUri === '') {
            throw new RuntimeException('MyInfo V5 redirect URI is not configured.');
        }
        ...
        $parResponse = (new MyinfoV5RequestSender)->send($parRequest, 'par', true, $this);
        ...
        $authorizationUrl = $metadata['authorization_endpoint'].'?'.http_build_query([
            'client_id' => config('laravel-myinfo-sg-v5.client_id'),
            'request_uri' => $requestUri,
        ]);

        if (config('laravel-myinfo-sg-v5.debug_mode')) {
            Log::debug('-- MyInfo V5 Authorise Call --');
    ...
    {
        $latestState = session()->pull(config('laravel-myinfo-sg-v5.state_session_key'));
        ...
        $codeVerifier = session(config('laravel-myinfo-sg-v5.code_verifier_session_key'));
    ...
    public function completeAuthorization(Request $request): VerifiedTokenSet
    {
        ...
        if (! is_string($clientId) || $clientId === '') {
            throw new RuntimeException('MyInfo V5 client ID is not configured.');
        }
    ...
    public function getUser(string $accessToken): GetUserResponse
    {
        ...
        $response = (new MyinfoV5RequestSender)->sendWithRequestFactory(
    ...
    public function getVerifiedUserInfo(VerifiedTokenSet $tokenSet): VerifiedUserInfo
    {
        ...
        if (! is_string($clientId) || $clientId === '') {
            throw new InvalidUserInfoException('MyInfo V5 client ID is not configured.');
        }
    ...
    ): array
    {
        ...
        if (! is_string($issuerUri) || $issuerUri === '') {
            throw new RuntimeException('MyInfo V5 issuer URI is not configured.');
        }
    ...
    private function transactionStore(): AuthorizationTransactionStore
    {
        ...
        if (! $session instanceof Session) {
            throw new RuntimeException('A Laravel session is required for MyInfo V5 authorization.');
        }
    ...
    {
        $algorithm = config('laravel-myinfo-sg-v5.dpop_signing_alg', 'ES256');

        if (! is_string($algorithm) || $algorithm === '') {
            throw new RuntimeException('MyInfo V5 DPoP signing algorithm is invalid.');
        }
    ...
    private function sendAccessTokenResponse(
        ...
        return (new MyinfoV5RequestSender)->send($request, 'token', true, $this);
    }
