...
final readonly class AuthorizationTransactionStore
{
    ...
    {
        $sessionKey = config('laravel-myinfo-sg-v6.transaction_session_key');

        if (! is_string($sessionKey) || $sessionKey === '') {
            throw new RuntimeException('MyInfo V6 transaction session key is not configured.');
        }
    ...
    {
        $ttl = config('laravel-myinfo-sg-v6.transaction_ttl_seconds');

        if (! is_int($ttl) || $ttl < 1) {
            throw new RuntimeException('MyInfo V6 transaction TTL must be a positive integer.');
        }
