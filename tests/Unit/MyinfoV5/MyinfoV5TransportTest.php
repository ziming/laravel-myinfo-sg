...
class MyinfoV5TransportTest extends TestCase
{
    ...
    {
        config()->set('laravel-myinfo-sg-v5.connect_timeout_seconds', '1.5');
        config()->set('laravel-myinfo-sg-v5.request_timeout_seconds', 9);
        config()->set('laravel-myinfo-sg-v5.safe_read_max_attempts', '3');
        config()->set('laravel-myinfo-sg-v5.safe_read_retry_delay_milliseconds', '0');
    ...
    public function test_invalid_transport_configuration_fails_before_building_a_request(
        ...
        config()->set("laravel-myinfo-sg-v5.{$key}", $value);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage("MyInfo V5 transport configuration [{$key}] is invalid.");
    ...
    public function test_exhausted_retryable_responses_map_to_the_sanitized_transport_boundary(
        ...
        try {
            (new MyinfoV5RequestSender)->send(
            ...
            $this->assertSame('The MyInfo V5 transport request could not be completed.', $exception->getMessage());
    ...
    public function test_safe_reads_retry_only_the_allowed_http_statuses(int $status): void
    {
        ...
        $this->expectException(MyinfoV5TransportException::class);
    ...
    {
        $exception = new MyinfoV5TransportException(
            'token',
            true,
        ...
        $this->assertSame(
            '{"message":"The MyInfo V5 transport request could not be completed.","endpoint":"token","restart_authorization":true}',
            $json,
        );
    }
    ...
    public function test_par_and_token_requests_never_retry_retryable_responses(): void
    {
        ...
        config()->set('laravel-myinfo-sg-v5.client_id', 'transport-client-id');
        config()->set('laravel-myinfo-sg-v5.redirect_uri', 'https://client.example/callback');
        config()->set('laravel-myinfo-sg-v5.scopes', 'openid');
        config()->set('laravel-myinfo-sg-v5.chosen_jwks_sig_kid', 'transport-client-signing-key');
        ...
        foreach ($requests as $endpoint => $request) {
            try {
                (new MyinfoV5RequestSender)->send($request, $endpoint, true);
...
final class TestTransportRequest extends MyinfoV5Request
{
